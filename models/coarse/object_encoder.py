    ...
    def forward(self, objects: List[Object3d], object_points):
        ...
        if "color" in self.args.use_features:
            if "color_embed" in self.args and self.args.color_embed:
                ...
                color_embedding = torch.nan_to_num(color_embedding.float(), nan=0.0, posinf=0.0, neginf=0.0)
                embeddings.append(F.normalize(color_embedding, p=2, dim=-1, eps=1e-6))
            else:
                colors = []
        ...
        if "position" in self.args.use_features:
            ...
            pos_embedding = torch.nan_to_num(pos_embedding.float(), nan=0.0, posinf=0.0, neginf=0.0)
            embeddings.append(F.normalize(pos_embedding, p=2, dim=-1, eps=1e-6))
        ...
        if "num" in self.args.use_features:
            ...
            num_points_tensor = torch.tensor(num_points, dtype=torch.float, device=self.get_device()).unsqueeze(-1)
            num_points_tensor = (num_points_tensor - self.num_mean) / max(float(self.num_std), 1e-6)
            num_points_tensor = torch.clamp(num_points_tensor, min=-10.0, max=10.0)
            num_points_tensor = torch.nan_to_num(num_points_tensor, nan=0.0, posinf=10.0, neginf=-10.0)
            num_points_embedding = self.num_encoder(num_points_tensor.float())
            num_points_embedding = torch.nan_to_num(num_points_embedding.float(), nan=0.0, posinf=0.0, neginf=0.0)
            embeddings.append(F.normalize(num_points_embedding, p=2, dim=-1, eps=1e-6))


        if len(embeddings) > 1:
            embeddings = self.mlp_merge(torch.cat(embeddings, dim=-1).float())
        else:
