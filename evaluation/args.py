...
def parse_arguments():
    ...
    parser.add_argument(
        "--path_coarse_secondary",
        type=str,
        default="",
        help="Optional second coarse checkpoint for score/rank fusion during evaluation.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="single",
        choices=["single", "zscore_sum", "rrf"],
        help="How to combine two coarse checkpoints when path_coarse_secondary is provided.",
    )
    parser.add_argument(
        "--coarse_weight_primary",
        type=float,
        default=1.0,
        help="Primary coarse model weight for score/rank fusion.",
    )
    parser.add_argument(
        "--coarse_weight_secondary",
        type=float,
        default=1.0,
        help="Secondary coarse model weight for score/rank fusion.",
    )
    parser.add_argument(
        "--rrf_k",
        type=int,
        default=60,
        help="Reciprocal-rank fusion constant when coarse_fusion=rrf.",
    )
    parser.add_argument(
        "--rerank_topn",
        type=int,
        default=0,
        help="If > 0, rerank the top-N coarse candidates using symbolic object/color overlap features.",
    )
    parser.add_argument(
        "--rerank_label_weight",
        type=float,
        default=0.6,
        help="Weight for mentioned object-label coverage in reranking.",
    )
    parser.add_argument(
        "--rerank_color_weight",
        type=float,
        default=1.0,
        help="Weight for mentioned color+label pair coverage in reranking.",
    )
    parser.add_argument(
        "--rerank_base_weight",
        type=float,
        default=1.0,
        help="Weight for the original coarse score inside the reranker.",
    )
    parser.add_argument(
        "--use_model_reranker",
        action="store_true",
        help="Use the coarse checkpoint's trainable reranker head instead of fixed symbolic weights.",
    )
    ...
    parser.add_argument("--prealign_pointnet_path", type=str, default="")
    parser.add_argument("--prealign_color_path", type=str, default="")
    parser.add_argument("--prealign_mlp_path", type=str, default="")
    ...
    if args.path_coarse_secondary:
        assert osp.isfile(args.path_coarse_secondary)
    if args.path_fine:
